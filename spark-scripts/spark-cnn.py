...
sparkcontext = pyspark.SparkContext.getOrCreate(
    conf=(pyspark.SparkConf().setAppName("FinalProject"))
)
...
df = spark.read.csv('/opt/bitnami/spark/data/cnn.csv', schema=schema, header=True)
...
df = df.filter(~df.text.like("1. %"))
df.show(5)
print(df.count())

# df.toPandas().to_csv('/opt/bitnami/spark/data/cnn_spark.csv', index=False)
...
print(merged_df)
merged_df.to_csv('/opt/bitnami/spark/data/cnn_spark.csv', index=False)
